...
def build_simple_cnn(
    ...
    s = layers.Dense(
        scalar_units, activation=activation, name="scalar_dense"
    )(scalar_input)
...
def build_tcn(
    ...
    for s in range(nb_stacks):
        for d in dilations:
            ...
            x = _norm_layer(
                normalization, name=f"tcn_norm{s}_dil{d}"
            )(x)
            x = _activation_layer(activation)(x)
...
def build_lstm(
    ...
    for i in range(lstm_layers):
        ...
        x = (
            ...
            else layers.Bidirectional(lstm, name=f"bilstm_{i}")(x)
        )
        x = _norm_layer(normalization, name=f"lstm_norm_{i}")(x)
...
def build_transformer1d(
    ...
    for i in range(num_blocks):
        ...
        ff = layers.Dense(ff_dim, activation=activation, name=f"ff_{i}_0")(x)
...
def _spectral_regression_head(
    x: tf.Tensor,
    signals_input: tf.Tensor,
    *,
    scalar_dim: int,
    scalar_units: int,
    dense_units: int,
    ...
    """Attach the shared environmental-scalar regression head."""
    x = layers.GlobalAveragePooling1D(name="gap")(x)
    scalar_input = layers.Input(shape=(scalar_dim,), name="scalars")
    scalars = layers.Dense(
        scalar_units,
        activation=activation,
        kernel_regularizer=regularizers.l2(l2),
        name="scalar_dense",
    )(scalar_input)
    scalars = layers.Dropout(dropout, name="scalar_dropout")(scalars)
    combined = layers.concatenate([x, scalars], name="concat")
    ...
    combined = layers.Dropout(dropout, name="drop_0")(combined)
    output = layers.Dense(
        num_outputs,
        activation="linear",
        name="concentrations",
    ...
    return models.Model(
        inputs=[signals_input, scalar_input],
        outputs=output,
        name=model_name,
    )


def _inception_spectral_module(
    x: tf.Tensor,
    *,
    filters: int,
    ...
    """Multi-receptive-field module adapted from DeepSpectra/InceptionTime."""
    ...
    kernel_sizes = (
        kernel_size,
        max(3, kernel_size // 2 | 1),
        max(3, kernel_size // 4 | 1),
    )
    branches = [
        layers.Conv1D(
            filters,
    ...
    result = layers.concatenate(branches, name=f"{name}_concat")
    result = _norm_layer(normalization, name=f"{name}_norm")(result)
    result = _activation_layer(activation)(result)
    return layers.SpatialDropout1D(
        dropout, name=f"{name}_dropout"
    )(result)


def build_inception_spectra(
    input_shape: tuple[int, int],
    scalar_dim: int,
    modules: int = 3,
    filters: int = 16,
    ...
    """Compact Inception regressor for multi-scale quantitative spectra."""
    if modules <= 0 or residual_interval <= 0:
        raise ValueError("modules and residual_interval must be positive")
    signals_input = layers.Input(shape=input_shape, name="signals")
    x = layers.Conv1D(
        filters,
        7,
    ...
    for index in range(modules):
        ...
        if use_residual and (index + 1) % residual_interval == 0:
            output_channels = int(x.shape[-1])
            shortcut = residual
            if int(shortcut.shape[-1]) != output_channels:
                shortcut = layers.Conv1D(
                    output_channels,
                    1,
                    padding="same",
                    use_bias=False,
                    kernel_regularizer=regularizers.l2(l2),
                    name=f"residual_projection_{index}",
                )(shortcut)
                shortcut = _norm_layer(
                    normalization,
                    name=f"residual_projection_norm_{index}",
                )(shortcut)
            x = layers.Add(name=f"residual_add_{index}")([x, shortcut])
            x = _activation_layer(activation)(x)
            residual = x

    return _spectral_regression_head(
        x,
        signals_input,
...
def _dilated_residual_block(
    ...
    *,
    ...
    x = _norm_layer(normalization, name=f"{name}_norm1")(x)
    x = _activation_layer(activation)(x)
    x = layers.SpatialDropout1D(dropout, name=f"{name}_dropout")(x)
    x = layers.Conv1D(
        filters,
        kernel_size,
        padding="same",
    ...
    if int(shortcut.shape[-1]) != filters:
        shortcut = layers.Conv1D(
            filters, 1, padding="same", use_bias=False, name=f"{name}_shortcut"
        )(shortcut)
    x = layers.Add(name=f"{name}_add")([x, shortcut])
    return _activation_layer(activation)(x)
...
def build_dilated_resnet1d(
    ...
    """Non-causal dilated ResNet for fixed-position spectroscopy regression."""
    if blocks <= 0 or dilation_depth <= 0:
        raise ValueError("blocks and dilation_depth must be positive")
    signals_input = layers.Input(shape=input_shape, name="signals")
    x = layers.Conv1D(
        filters,
        kernel_size,
    ...
    for index in range(blocks):
        x = _dilated_residual_block(
            x,
            filters=filters,
            kernel_size=kernel_size,
            dilation_rate=2 ** (index % dilation_depth),
            dropout=dropout,
            l2=l2,
            normalization=normalization,
            activation=activation,
            name=f"dilated_block_{index}",
        )
    return _spectral_regression_head(
        x,
        signals_input,
